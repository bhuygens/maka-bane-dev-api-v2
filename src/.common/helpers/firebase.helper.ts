...
export class FirebaseHelper {
  ...
  static async uploadImagesToFirebase(images: string[], itemName: string, type: string): Promise<string[]> {
    ...
    images.forEach((image, x = 0) => {
      ...
      const url = `https://firebasestorage.googleapis.com/v0/b/maka-bane-dev/o/${encodeURIComponent(remoteFilePath)}?alt=media&token=${uuid}`;
      ...
      file.save(imageByteArray, {
        ...
        .catch((err: any) => {
          ErrorManager.badRequestException(`Unable to upload encoded file ${err}`);
        });
    });
